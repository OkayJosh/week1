...
include "/home/etentlabs/zku-week1/Q3/contracts/circuits/RangeProof.circom";
//include here is an absolute path, open to the usage of relative paths
...
template sudoku() {
    ...
    component puzzle_range1[9][9];
    component puzzle_range2[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
             puzzle_range1[i][j] = RangeProof(32);
             puzzle_range1[i][j].in <== puzzle[i][j];
             puzzle_range1[i][j].range[0] <== 0;
             puzzle_range1[i][j].range[1] <== 9;

             puzzle_range2[i][j] = RangeProof(32);
             puzzle_range2[i][j].in <== solution[i][j];
             puzzle_range2[i][j].range[0] <== 0;
             puzzle_range2[i][j].range[1] <== 9;
